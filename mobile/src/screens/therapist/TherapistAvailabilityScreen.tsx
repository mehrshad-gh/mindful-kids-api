...
function formatSlot(slot: TherapistSlot): string {
...
function createdByLabel(slot: TherapistSlot): string {
...
export function TherapistAvailabilityScreen() {
  ...
  const handleDelete = useCallback(
    (slot: TherapistSlot) => {
      ...
      Alert.alert('Delete slot?', formatSlot(slot), [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteSlot(slot.id, slot.version ?? undefined);
              await load();
            } catch (e: unknown) {
              const msg =
                e && typeof e === 'object' && 'response' in e
                  ? (e as { response?: { status?: number; data?: { error?: string } } }).response?.status === 409
                    ? 'Slot was modified; please refresh and try again.'
                    : (e as { response?: { data?: { error?: string } } }).response?.data?.error
                  : null;
              Alert.alert('Error', msg ?? 'Could not delete slot.');
            }
          },
        },
      ]);
  ...
  return (
    <ScreenLayout>
      ...
      <Text style={styles.subtitle}>Add time slots when you’re available. Parents can book these slots.</Text>

      <Card style={styles.card}>
        <Text style={styles.cardLabel}>New slot</Text>
        <Text style={styles.timezoneLabel}>Timezone: {timezone}</Text>
        <Text style={styles.timezoneHint}>Times below are in your local time ({timezoneLabel}).</Text>
        ...
        <ScrollView style={styles.slotList} showsVerticalScrollIndicator={false}>
          {slots.map((slot) => (
            <Card key={slot.id} style={styles.slotCard}>
              ...
              <Text style={styles.slotStatus}>{slot.status} · {createdByLabel(slot)}</Text>
              {slot.status === 'open' ? (
...
const styles = StyleSheet.create({
  ...
  slotStatus: { ...typography.caption, color: colors.textSecondary, marginTop: spacing.xs },
